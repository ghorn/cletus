...
callback
  :: Serialize a => (String -> Zmq.Packed -> IO ()) -> String -> a -> IO Bool
callback sendMsg channelName stuff = do
  sendMsg channelName (Zmq.encodeSerial stuff)
  return True

withCallback :: Serialize a
                => String -> String -> (((DynCollTraj a, CollTrajMeta) -> IO Bool) -> IO b) -> IO b
withCallback url channelName userFun =
  Zmq.withContext $ \context ->
    Zmq.withPublisher context url $ \sendMsg ->
      userFun (callback sendMsg channelName)
...
main = do
  ...
  withCallback chanDynoPlot "dynoplot" $ \cb -> do
